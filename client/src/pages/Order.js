...
export const Order = observer(() => {
  ...
  const [item, setItem] = useState([
    { basketId: 0, deviceId: 0, name: null, price: 0, image: null },
  ]);
  ...
  return (
    <div className="d-flex justify-content-between">
      <Col className="pl-5" md={6}>
        <h2 style={{ marginLeft: "200px", marginTop: "30px" }}>
          Оформление заказа
        </h2>
        ...
        <Form>
            ...
            <Col>
                ...
                <Form.Control value={phone} onChange={e => setPhone(e.target.value)} style={{width: "300px"}} type="number" placeholder="Введите номер телефона..." />
            </Col>
        ...
        {item.reduce((a, x) => (a += x.price), 0) === 0 || name === '' || surname === '' || region === '' || city === '' || phone === 0 || delivery === '' || department === 0 || delivery === '' || payment === '' ? <Button disabled className="mt-5" size="lg">Подтвердить</Button> : <Button onClick={() => setSuccessVisible(true)} className="mt-5" size="lg">Подтвердить</Button> }
        </Form>
      </Col>

      <Col className="mt-5 pr-5" md={6}>
        {item.map((inf, index) => (
          <OrderItem
      ...
      <Submit user={user} item={item} name={name} surname={surname} phone={phone} region={region} city={city} delivery={delivery} department={department} delivery={delivery} show={successVisible} payment={payment} onHide={() => setSuccessVisible(false)}></Submit>
    </div>
  );
});
